...
- Supported products: ChatGPT and Codex surfaces that support universal plugins
- Execution: not exposed in the first public release
- Local files: never available to the hosted integration

Before release, confirm the registered app ID from an OpenAI account outside the development workspace, current OpenAI requirements, and the frozen hosted build. Then run repository and plugin validation and exercise the positive and negative reviewer cases.
...
Keep the committed manifest version equal to the package version above. The GitHub release tag must be `v<package-version>` and must point at the exact commit that passed the external-account acceptance test.
